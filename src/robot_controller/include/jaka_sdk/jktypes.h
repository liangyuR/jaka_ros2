 ...
 * @brief move mode
 */
typedef enum { ABS = 0, INCR, CONTINUE, STOP } MoveMode;
 ...
 */
typedef struct {
  int id; ///< motion cmd id, range limit: [0, 5000], set to -1 if you want
          ///< controller to set automatically
  ...
  double tol; ///< tolerance, used for blending. set to 0 if you want to reach a
              ///< fine point
} MoveJParam;
...
typedef struct {
  ...
  JointMonitorData
      jointMonitorData[6]; ///< Monitoring data of the robot's six joints
} RobotMonitorData;
...
typedef struct {
  ...
  double actTorque[6];  ///< The actual contact force value of the torque sensor
                        ///< (when Initialize is checked) or the raw reading
                        ///< value (when Do Not Initialize is checked)
  double torque[6];     ///< Torque sensor raw reading value
  double realTorque[6]; ///< The actual contact force value of the torque sensor
                        ///< (does not change with the initialization options)
} TorqSensorMonitorData;

/**
 * @brief Robot status monitoring data, use the get_robot_status function to
 * update the robot status data
 */
typedef struct {
  int errcode; ///< Error number when the robot runs into an error. 0 means
               ///< normal operation, and others mean abnormal operation.
  int inpos; ///< The robot movement is in place, 0 means not in place, 1 means
             ///< in place
  int powered_on; ///< Whether the robot is powered on, 0 means no power, 1
                  ///< means power on
  int enabled;    ///< Flag indicating whether the robot is enabled, 0 means not
                  ///< enabled, 1 means enabled
  double rapidrate;    ///< Robot movement ratio
  int protective_stop; ///< Whether the robot detects a collision, 0 means no
                       ///< collision is detected, 1 means a collision is
                       ///< detected
  int emergency_stop;  ///< Whether the robot stops suddenly, 0 means no sudden
                       ///< stop, 1 means sudden stop
  int dout[256];       ///< Digital output signal of the robot control cabinet,
                       ///< dout[0] is the number of signals
  int din[256];    ///< Digital input signal of robot control cabinet, din[0] is
                   ///< the number of signals
  double ain[256]; ///< Robot control cabinet analog input signal, ain[0] is the
                   ///< number of signals
  double aout[256]; ///< Robot control cabinet analog output signal, aout[0] is
                    ///< the number of signals
  int tio_dout[16]; ///< The digital output signal of the robot end tool,
                    ///< tio_dout[0] is the number of signals
  int tio_din[16];  ///< The digital input signal of the robot end tool,
                    ///< tio_din[0] is the number of signals
  double tio_ain[16]; ///< Robot end tool analog input signal, tio_ain[0] is the
                      ///< number of signals
  ...
  unsigned int current_tool_id;     ///< The tool coordinate system id currently
                                    ///< used by the robot
  double cartesiantran_position[6]; ///< The Cartesian space position of the
                                    ///< robot end
  double joint_position[6];         ///< Robot joint space position
  unsigned int on_soft_limit;   ///< Whether the robot is in limit position, 0
                                ///< means no limit protection is triggered, 1
                                ///< means limit protection is triggered
  unsigned int current_user_id; ///< The user coordinate system id currently
                                ///< used by the robot
  int drag_status; ///< Whether the robot is in the dragging state, 0 means not
                   ///< in the dragging state, 1 means in the dragging state
  RobotMonitorData robot_monitor_data; ///< Robot status monitoring data
  TorqSensorMonitorData
      torq_sensor_monitor_data; ///< Robot torque sensor status monitoring data
  int is_socket_connect; ///< Whether the connection channel between SDK and
                         ///< controller is normal, 0 means the connection
                         ///< channel is abnormal, 1 means the connection
                         ///< channel is normal
} RobotStatus;
...
typedef struct {
  ...
  BOOL inpos; ///< current motion CMD is done, you should always check queue
              ///< info at the same time
  BOOL err_add_line; /// fail to add motion CMD in case robot is already at
                     /// target position
...
typedef struct {
  ...
  double ft_user; ///< the force value to let robot move in MAX velocity, also
                  ///< naming as ft_damping
  double ft_rebound;   ///< rebound force, ability for robot to go back to init
                       ///< position
  double ft_constant;  ///< constant force
  int ft_normal_track; ///< normal vector track，0: disable，1: enable.
                       ///< deprecated, cannot set any more(always disabled).
} AdmitCtrlType;

typedef struct {
  std::vector<AdmitCtrlType> admit_ctrl;
} AdmitCtrlTypeList;
...
typedef enum { ActualData = 0, NormalData = 1, RealData = 2 } TorqueDataType;
...
* @param info remember to assign char array, no less than 1024 bytes
                                the feedback info contains at least 3 data part:
"section", "key", "data" which is packed as json. The "data" part represent the
current(new) value
*/
