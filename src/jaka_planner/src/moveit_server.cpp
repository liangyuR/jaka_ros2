...
typedef rclcpp_action::Server<control_msgs::action::FollowJointTrajectory>
    Server;
...
bool jointStates(const JointValue &joint_pose) {
  ...
  for (int i = 0; i < 6; i++) {
    ...
    bool ret =
        (target_deg - 0.2 < actual_deg) && (actual_deg < target_deg + 0.2);
    joint_state = joint_state && ret;
  }

  RCLCPP_INFO(rclcpp::get_logger("jointStates"),
              "Whether the robot has reached the target position: %d",
              joint_state);
  return joint_state;
}

// Handle a new FollowJointTrajectory goal
void goalCb(const shared_ptr<rclcpp_action::ServerGoalHandle<
  ...
  if (point_num == 0) {
    RCLCPP_ERROR(rclcpp::get_logger("goalCb"),
                 "Trajectory has no points. Aborting goal.");
    goal_handle->abort(
        make_shared<control_msgs::action::FollowJointTrajectory::Result>());
    return;
  }
  ...
  for (int i = 1; i < point_num; i++) {
    ...
    RCLCPP_INFO(rclcpp::get_logger("goalCb"),
                "The return status of servo_j: %d", sdk_res);
    RCLCPP_INFO(rclcpp::get_logger("goalCb"),
                "For point no.: %d, Accepted joint angle: %f %f %f %f %f %f, "
                "dt=%f, step_num=%d",
                i, joint_pose.jVal[0], joint_pose.jVal[1], joint_pose.jVal[2],
                joint_pose.jVal[3], joint_pose.jVal[4], joint_pose.jVal[5], dt,
                step_num);
    ...
    //     RCLCPP_INFO(rclcpp::get_logger("goalCb"), "Servo Mode Disable, motion
  ...
  while (rclcpp::ok()) {
    if (jointStates(joint_pose)) {
      robot.servo_move_enable(false);
      RCLCPP_INFO(rclcpp::get_logger("goalCb"),
                  "Servo Mode Disable: Target Reached");
      RCLCPP_INFO(rclcpp::get_logger("goalCb"),
                  "==============Motion stops or reaches the target "
                  "position==============");
      break;
    }
  ...
  //     robot.motion_abort();
  //     robot.servo_move_enable(false);
  //     RCLCPP_INFO(rclcpp::get_logger("goalCb"), "Servo Mode Disabled, Motion
  //     Canceled"); auto result =
  //     make_shared<control_msgs::action::FollowJointTrajectory::Result>();
  //     goal_handle->canceled(result);
  //     return;
...
// Publish the robot's joint states to /joint_states (for RViz / MoveIt
// feedback)
...
int main(int argc, char *argv[]) {
  ...
  RCLCPP_INFO(rclcpp::get_logger("moveit_server"), "Robot IP: %s, Model: %s",
              robot_ip.c_str(), robot_model.c_str());
  ...
  auto moveit_server =
      rclcpp_action::create_server<control_msgs::action::FollowJointTrajectory>(
          node, "/jaka_" + robot_model + "_controller/follow_joint_trajectory",
          // Goal callback
