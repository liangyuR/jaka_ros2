...
// Define variable: the direction that was sent down the last time the jog was
// called
...
bool linear_move_callback(
    const shared_ptr<jaka_msgs::srv::Move::Request> request,
  ...
  Eigen::Vector3d Angaxis = {request->pose[3], request->pose[4],
                             request->pose[5]};
  ...
  int ret = robot.linear_move(&end_pose, MoveMode::ABS, TRUE, speed, accel, tol,
                              option_cond);
  switch (ret) {
  case 0:
    response->ret = 1;
...
bool joint_move_callback(
  ...
  int ret = robot.joint_move(&joint_pose, MoveMode::ABS, true, speed, accel,
                             tol, option_cond);
  switch (ret) {
  case 0:
    response->ret = 1;
...
bool jog_callback(const shared_ptr<jaka_msgs::srv::Move::Request> request,
  ...
  // request.index 如果是笛卡尔空间
  // 就是x+,x-,y+,y-,z+,z-,rx+,rx-,ry+,ry-,rz+,rz-
  ...
  case 2:
    ...
    break;
  default:
    RCLCPP_INFO(rclcpp::get_logger("jog_callback"),
                "Coordinate system input error, please re-enter");
    return true;
  }
  // 4. Determine the direction of velocity
  // Determine whether robot motion (articulated or Cartesian) is in a positive
...
bool servo_move_enable_callback(
    const shared_ptr<jaka_msgs::srv::ServoMoveEnable::Request> request,
...
bool servo_p_callback(
    const shared_ptr<jaka_msgs::srv::ServoMove::Request> request,
...
bool stop_move_callback(
  ...
  switch (ret) {
  case 0:
    RCLCPP_INFO(rclcpp::get_logger("stop_move_callback"),
                "stop_move has been executed");
    break;
  default:
    RCLCPP_INFO(rclcpp::get_logger("stop_move_callback"), "error occurred: %s",
                mapErr[ret].c_str());
    return false;
    ;
  }
...
bool set_toolFrame_callback(
    const shared_ptr<jaka_msgs::srv::SetTcpFrame::Request> request,
...
bool set_userFrame_callback(
    const shared_ptr<jaka_msgs::srv::SetUserFrame::Request> request,
...
bool set_payload_callback(
    const shared_ptr<jaka_msgs::srv::SetPayload::Request> request,
...
bool set_collisionLevel_callback(
    const shared_ptr<jaka_msgs::srv::SetCollision::Request> request,
  ...
  case 0:
    response->ret = 1;
    response->message =
        "Collision level" + to_string(collision_level) + " has been executed";
    break;
  default:
    response->ret = 0;
    response->message = "error occurred:" + mapErr[ret];
...
bool get_io_callback(const shared_ptr<jaka_msgs::srv::GetIO::Request> request,
  ...
  // This part is redundant but is here to prevent reaching the end without a
  // return
  return true;
}
...
/*
bool clear_error_callback(const shared_ptr<jaka_msgs::srv::ClearError::Request>
request, shared_ptr<jaka_msgs::srv::ClearError::Response> response)
{
...
void tool_position_callback(
    const rclcpp::Publisher<geometry_msgs::msg::TwistStamped>::SharedPtr
        &tool_position_pub) {
  // Check if publisher is valid
  if (!tool_position_pub) {
    RCLCPP_ERROR(rclcpp::get_logger("jaka_driver"),
...
void joint_position_callback(
    const rclcpp::Publisher<sensor_msgs::msg::JointState>::SharedPtr
        &joint_position_pub) {
...
void robot_states_callback(
    const rclcpp::Publisher<jaka_msgs::msg::RobotMsg>::SharedPtr
        &robot_states_pub) {
  ...
  // else if(in_pos && programstate == PROGRAM_IDLE &&
  // (!robotstatus.drag_status))
...
void get_conn_scoket_state() {
  ...
  while (rclcpp::ok()) {
    ...
    //     "get_robot_status error!!!");
    // }
    // else if(!robot_status.is_socket_connect)
    // {
    //     RCLCPP_ERROR(rclcpp::get_logger("get_conn_socket_state"), "connect
    //     error!!!");
    // }

    if (ret) {
      RCLCPP_ERROR(rclcpp::get_logger("get_conn_socket_state"),
                   "Connection error or get_joint_position failed, error_code: "
                   "%d, error: %s",
                   ret, mapErr[ret].c_str());
    }
...
int main(int argc, char *argv[]) {
  ...
  auto linear_move_service = node->create_service<jaka_msgs::srv::Move>(
      "/jaka_driver/linear_move", &linear_move_callback);
  // 1.2 Joint motion
  auto joint_move_service = node->create_service<jaka_msgs::srv::Move>(
      "/jaka_driver/joint_move", &joint_move_callback);
  // 1.3 Jog motion
  auto jog_service = node->create_service<jaka_msgs::srv::Move>(
      "/jaka_driver/jog", &jog_callback);
  // 1.4 Servo Position Control Mode Enable
  auto servo_move_enable_service =
      node->create_service<jaka_msgs::srv::ServoMoveEnable>(
          "/jaka_driver/servo_move_enable", &servo_move_enable_callback);
  // 1.5 Servo-mode motion in Cartesian space
  auto servo_p_service = node->create_service<jaka_msgs::srv::ServoMove>(
      "/jaka_driver/servo_p", &servo_p_callback);
  // 1.6 Joint space servo mode motion
  auto servo_j_service = node->create_service<jaka_msgs::srv::ServoMove>(
      "/jaka_driver/servo_j", &servo_j_callback);
  // 1.7 stop motion
  auto stop_move_service = node->create_service<std_srvs::srv::Empty>(
      "/jaka_driver/stop_move", &stop_move_callback);
  // 2.1 Setting tcp parameters
  auto set_toolframe_service =
      node->create_service<jaka_msgs::srv::SetTcpFrame>(
          "/jaka_driver/set_toolframe", &set_toolFrame_callback);
  // 2.2 Setting user coordinate system parameters
  auto set_userframe_service =
      node->create_service<jaka_msgs::srv::SetUserFrame>(
          "/jaka_driver/set_userframe", &set_userFrame_callback);
  // 2.3 Set the center of gravity parameters of the robot arm load
  auto set_payload_service = node->create_service<jaka_msgs::srv::SetPayload>(
      "/jaka_driver/set_payload", &set_payload_callback);
  // 2.4 Set free drive mode
  auto drag_move_service = node->create_service<std_srvs::srv::SetBool>(
      "/jaka_driver/drag_move", &drag_mode_callback);
  // 2.5 Set collision sensitivity
  auto set_collisionlevel_service =
      node->create_service<jaka_msgs::srv::SetCollision>(
          "/jaka_driver/set_collisionlevel", &set_collisionLevel_callback);
  // 2.6 Set IO
  auto set_io_service = node->create_service<jaka_msgs::srv::SetIO>(
      "jaka_driver/set_io", &set_io_callback);
  // 2.7 Get IO
  auto get_io_service = node->create_service<jaka_msgs::srv::GetIO>(
      "jaka_driver/get_io", &get_io_callback);
  // 2.8 Find the positive solution
  auto get_fk_service = node->create_service<jaka_msgs::srv::GetFK>(
      "jaka_driver/get_fk", &get_fk_callback);
  // 2.9 Find the inverse solution
  auto get_ik_service = node->create_service<jaka_msgs::srv::GetIK>(
      "jaka_driver/get_ik", &get_ik_callback);

  // //3.1 End position pose status information reporting
  tool_position_pub = node->create_publisher<geometry_msgs::msg::TwistStamped>(
      "/jaka_driver/tool_position", 10);
  // //3.2 Joint status information reporting
  joint_position_pub = node->create_publisher<sensor_msgs::msg::JointState>(
      "/jaka_driver/joint_position", 10);
  // //3.3 Report robot event status information
  robot_state_pub = node->create_publisher<jaka_msgs::msg::RobotMsg>(
      "/jaka_driver/robot_states", 10);

  // Automatically stop robot jog and motion
  auto stop_jog =
      node->create_wall_timer(chrono::seconds(3), stop_jog_callback);
